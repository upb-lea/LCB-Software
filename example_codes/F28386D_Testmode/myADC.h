...
extern uint16_t ADCINA3;
extern uint16_t ADCINB3;
extern uint16_t ADCINC3;
extern uint16_t ADCIND3;
...
extern void AdcAInit(uint32_t resolution,
										 uint32_t signalMode);
// Funktion initialisiert den ADC (Modul B)
extern void AdcBInit(uint32_t resolution,
										 uint32_t signalMode);
// Funktion initialisiert den ADC (Modul C)
extern void AdcCInit(uint32_t resolution,
										 uint32_t signalMode);
// Funktion initialisiert den ADC (Modul D)
extern void AdcDInit(uint32_t resolution,
										 uint32_t signalMode);

// Interrupt-Service-Routine für den ADCINT1 (Modul A)
__interrupt void AdcAInt1ISR(void);
// Interrupt-Service-Routine für den ADCINT1 (Modul B)
__interrupt void AdcBInt1ISR(void);
// Interrupt-Service-Routine für den ADCINT1 (Modul C)
__interrupt void AdcCInt1ISR(void);
// Interrupt-Service-Routine für den ADCINT1 (Modul D)
