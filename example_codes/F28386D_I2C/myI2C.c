...
///					und die Datengröße auf 8 Bit. Der I2C-Interrupt wird eingeschaltet und die ISR auf
...
void I2cInitA(uint32_t clock)
{
    ...
    __asm(" RPT #4 || NOP");
		...
		// die Werte für die Register I2CCLKH und I2CCLKL nicht korrekt berechnen.
		...
		// FIFO-Modus einschalten (gilt sowohl für den Schreib- als auch den Lesebetrieb)
		I2caRegs.I2CFFTX.bit.I2CFFEN = 1;
		// Wenn der Wert im Feld I2CFFTX.TXFFST gleich oder kleiner als der Wert in
		// I2CFFTX.TXFFIL ist, wird ein Interrupt ausgelöst. Der Wert I2CFFTX.TXFFST
		...
		// die Anzahl der vollständig gesendeten Bytes wieder, da sich das ent-
		// sprechende Byte noch im Sende-Shift-Register befindet und Bit für Bit
		// ausgesendet wird, während der Wert bereits um 1 reduziert wurde. Somit
		// wird auch der Interrupt ausgelöst, bevor das Byte vollständig gesendet wurde
		I2caRegs.I2CFFTX.bit.TXFFIL = 0;
		// Wenn der Wert im Feld I2CFFRX.RXFFST gleich oder größer als der Wert in
		// I2CFFRX.RXFFIL ist, wird ein Interrupt ausgelöst. Der Wert I2CFFRX.RXFFST
		// speichert die Zahl an Bytes, die im Empfangs-FIFO stehen. Anders als beim
		// Sendevorgang wird der Interrupt also genau zu dem Zeitpunkt ausgelöst,
		// wenn das letzte Byte vollständig empfangen wurde
    ...
    I2cInitBufferReadA();
    I2cInitBufferWriteA();
    // Steuervariable für die Puffer-Verwaltung initialisieren
...
///					an. Die zu schreibenden Daten müssen von der aufrufenden Stelle in das globale Array
///					"i2cBufferWrite" kopiert werden. Es können maximal 16 Bytes geschrieben werden. Der
///					Rückgabwert ist "true" falls die Kommunikation gestartet wurde (keine vorangegegangene
...
///					an. Die gelesenen Daten können von der aufrufenden Stelle aus dem globale Array
///					"i2cBufferRead" gelesen werden. Es können maximal 16 Bytes gelesen werden. Der
///					Rückgabwert ist "true" falls die Kommunikation gestartet wurde (keine vorangegegangene
///					Kommunikation ist aktiv und der Bus ist frei), andernfalls ist er "false".
///
/// @param  uint16_t slaveAddress, uint16_t numberOfBytes
...
///					jeweils ohne die Slave-Adresse). Es können maximal 16 Bytes geschreiben bzw. gelesen
...
__interrupt void I2cISRA(void)
{
		...
		// nur möglich nach dem Senden der Slave-Adresse
